...
calc_ea (REGS *r, BYTE modrm)
{
	...
	case 0:
		...
		if (rm == 6) {
			addr = (MEMORY[ipidx])
			     | (MEMORY[SAFE_IP_PLUS(1)] << 8);
			ipidx = SAFE_IP_PLUS(2);
	...
	case 2:
		/* [rm + disp16 */
		disp = MEMORY[ipidx] | (MEMORY[SAFE_IP_PLUS(1)] << 8);
		ipidx = SAFE_IP_PLUS(2);
		break;
	case 3:
		/* reg */
...
exec_add_r8 (REGS *r)
{
	...
	if (mod == 3) {
		...
		*dst = res & 0xFF;
		...
		addr = calc_ea(r, modrm);

		if (addr == (memptr_t)-1 && modrm != 6) {
...
exec_mov_r16 (REGS *r)
{
	...
	if (mod == 3) {
		...
		*dst = *src;
		...
		val = MEMORY[addr] | (MEMORY[SAFE_PLUS(addr, 1)] << 8);
		*dst = val;
	}
}
...
exec_ff (REGS *r)
{
	...
	case 0:
		if (mod == 3) {
			...
			MEMORY[SAFE_PLUS(addr, 1)] = (val >> 8) & 0xFF;
		}
		break;
	case 1:
		if (mod == 3) {
...
dos_sys_read INT21 (REGS *r)
{
	...
	memcpy(&MEMORY[SAFE_PLUS(addr, 2)], line, bytes_read);
	free(line);
...
void
runcom (REGS *r, int fd, size_t sz)
{
	BYTE ch, ch2, mod, reg, rm, *src, *dst, off, modrm;
	WORD off16, seg16, res, val;
	memptr_t addr;
	int rret = read(fd, &MEMORY[PRG_START], memsz - PRG_START);
	...
	while (true) {
		...
		case 0x70:
			...
			if (r->flags & OF)
				ipidx = SAFE_IP_PLUS((signed char)off);
			break;
		/* jno $imm8 */
		case 0x71:
			off = MEMORY[ipidx];
		...
		case 0xE9:
			off16 = (MEMORY[ipidx])
			      | (MEMORY[SAFE_IP_PLUS(1)] << 8);
			ipidx = SAFE_IP_PLUS(2);
			ipidx = SAFE_IP_PLUS((int16_t)off16);
			break;
		/* jmp far $imm16:imm16 */
		case 0xEA:
			off16 = (MEMORY[ipidx])
			      | (MEMORY[SAFE_IP_PLUS(1)] << 8);
			seg16 = (MEMORY[SAFE_IP_PLUS(2)])
			      | (MEMORY[SAFE_IP_PLUS(3)] << 8);
			r->CS = seg16;
			r->IP = off16;
